...
export function PayrollSummary({ timeEntries }: PayrollSummaryProps) {
  ...
  const totalRegular = timeEntries.reduce((sum, entry) => sum + entry.hoursWorked, 0)
  const totalOvertime = timeEntries.reduce((sum, entry) => sum + entry.overtime, 0)
  ...
  const estimatedPayroll = totalRegular * regularRate + totalOvertime * overtimeRate
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold">{Object.keys(userHours).length}</p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold">${estimatedPayroll.toLocaleString()}</p>
              ...
              {Object.entries(userHours).map(([userName, hours]) => (
                <div key={userName} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-medium">{(hours.regular + hours.overtime).toFixed(1)}h total</p>
