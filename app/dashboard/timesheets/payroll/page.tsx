...
export default async function PayrollReportPage() {
  ...
  const payrollData = timeEntries.reduce(
    (acc, entry) => {
      ...
      acc[userName].totalPay +=
        entry.hoursWorked * regularRate + entry.overtime * overtimeRate;
  ...
  const totalRegularHours = payrollSummary.reduce(
    (sum, emp: any) => sum + emp.regularHours,
    0
  );
  const totalOvertimeHours = payrollSummary.reduce(
    (sum, emp: any) => sum + emp.overtimeHours,
    0
  );
  const totalPayroll = payrollSummary.reduce(
    (sum, emp: any) => sum + emp.totalPay,
    0
  );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Employee hours and payroll calculations
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-2xl font-bold">
                  {(totalRegularHours + totalOvertimeHours).toFixed(1)}
                </p>
