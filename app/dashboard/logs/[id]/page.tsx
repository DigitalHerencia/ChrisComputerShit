...
export default async function DailyLogDetailPage({
  ...
  const createdByName = `${dailyLog.createdBy.firstName || ""} ${dailyLog.createdBy.lastName || ""}`.trim()
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">Daily Log - {format(dailyLog.date, "MMM d, yyyy")}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-foreground leading-relaxed">{dailyLog.workDone}</p>
            </CardContent>
          </Card>
          ...
          {dailyLog.notes && (
            <Card>
              ...
              <CardContent>
                <p className="text-foreground leading-relaxed">{dailyLog.notes}</p>
              </CardContent>
            </Card>
          )}
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="font-medium">{format(dailyLog.date, "EEEE, MMMM d, yyyy")}</p>
                </div>
              </div>
            ...
            <CardContent>
              <div className="space-y-3">
                <div>
                  ...
                  {dailyLog.project.location && (
                    <p className="text-sm text-muted-foreground">{dailyLog.project.location}</p>
                  )}
                </div>
                <Button asChild variant="outline" size="sm" className="w-full bg-transparent">
                  <Link href={`/dashboard/projects/${dailyLog.projectId}`}>View Project</Link>
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-sm text-muted-foreground">{format(dailyLog.createdAt, "MMM d, yyyy 'at' h:mm a")}</p>
            </CardContent>
          </Card>
        </div>
      </div>
